		...
		if(posX < tileX + tileW &&
   		 ...
   		 posY + this.squareHithox > tileY){
					if(tile.type == "tile"){
						if(this.y < center.y){
							if(this.x < center.x){
								if(!t && BR.y <= tileY+1){this.y = tileY-(this.squareHithox/2)};
								if(!l && BR.x <= tileX+1){this.x = tileX-(this.squareHithox/2)};
							} else {
								if(!t && BL.y <= tileY+1){this.y = tileY-(this.squareHithox/2)};
								if(!r && BL.x >= tileX+tileW-1){this.x = tileX+tileW+(this.squareHithox/2)};
							};
						} else {
							if(this.x < center.x){
								if(!b && TR.y >= tileY+tileW-1){this.y = (tileY+tileW)+(this.squareHithox/2)};
								if(!l && TR.x <= tileX+1){this.x = tileX-(this.squareHithox/2)};
							} else {
								if(!b && TL.y >= tileY+tileW-1){this.y = (tileY+tileW)+(this.squareHithox/2)};
								if(!r && TL.x >= tileX+tileW-2){this.x = tileX+tileW+(this.squareHithox/2)};
							};
						};
